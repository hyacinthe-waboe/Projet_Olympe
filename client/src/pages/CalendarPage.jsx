...
const messagesFr = {
  ...
  date: 'Date', time: 'Heure', event: 'Événement', noEventsInRange: 'Aucun RDV.',
};
...
const RdvModal = ({ isOpen, onClose, onSubmit, onDelete, clients, appelants, initialData, isEditMode, fixedClient }) => {
  const [formData, setFormData] = useState({ id: null, title: '', start: '', end: '', description: '', clientId: '', appelantId: '' });
  ...
  const handleSubmit = (e) => {
    ...
    if (startHour < 8 || startHour >= 18 || endHour < 8 || endHour > 18 || (endHour === 18 && endDate.getMinutes() > 0)) {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" required value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full border rounded px-3 py-2 mt-1" />
          </div>
          <div className="grid grid-cols-2 gap-4">
             <div><label className="block text-sm font-medium text-gray-700">Début</label><input type="datetime-local" required value={formData.start} onChange={e => setFormData({...formData, start: e.target.value})} className="w-full border rounded px-3 py-2 mt-1" /></div>
             <div><label className="block text-sm font-medium text-gray-700">Fin</label><input type="datetime-local" required value={formData.end} onChange={e => setFormData({...formData, end: e.target.value})} className="w-full border rounded px-3 py-2 mt-1" /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
                <label className="block text-sm font-medium text-gray-700">Médecin *</label>
                <select required value={formData.clientId} onChange={e => setFormData({...formData, clientId: e.target.value})} disabled={!!fixedClient} className={`w-full border rounded px-3 py-2 mt-1 ${fixedClient ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-white'}`}>
            ...
            <div>
                ...
                <select required value={formData.appelantId} onChange={e => setFormData({...formData, appelantId: e.target.value})} className="w-full border rounded px-3 py-2 mt-1 bg-white">
                    <option value="">Choisir...</option>
          ...
          <div className="flex justify-between pt-4">
             ...
             <div>
                 {isEditMode && (
                     <button type="button" onClick={handleDelete} className="px-4 py-2 bg-red-100 text-red-600 rounded hover:bg-red-200 flex items-center gap-2">
                         <FaTrash /> Supprimer
                     </button>
                 )}
             </div>
             ...
             <div className="flex">
                <button type="button" onClick={onClose} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded mr-2">Annuler</button>
        ...
        <div className="w-80 bg-gray-50 border-l border-gray-200 flex flex-col p-4 space-y-4 overflow-y-auto">
            <div className="relative">
                ...
                >
                    ...
                    {clients.map(c => <option key={c.id} value={c.id}>Agenda: Dr. {c.lastName}</option>)}
                </select>
                    ...
                    {clients.map(client => {
                        ...
                        return (
                            <div key={client.id} className="flex items-center gap-2 text-sm">
                                ...
                                ></span>
                                <span className="text-gray-700 font-medium truncate">Dr. {client.lastName}</span>
                            </div>
                        );
                    })}
                    {clients.length === 0 && <p className="text-sm text-gray-400 italic">Aucun médecin</p>}
                </div>
            </div>
        </div>
    );
...
export default function CalendarPage() {
  ...
  const fetchData = async () => {
        try {
            const resCl = await fetch('http://127.0.0.1:8000/api/admin/clients', { credentials: 'include' });
            ...
            if (resRdv.ok) {
                ...
                setEvents(data.map(evt => ({
                    ...evt, 
                    start: new Date(evt.start), end: new Date(evt.end),
  ...
  // 1. Clic sur une case vide -> CRÉATION
    const handleSelectSlot = ({ start }) => {
      ...
      setModalData({ start, end, title: '', clientId: selectedClient, appelantId: '' });
      setIsEditMode(false);
      setIsModalOpen(true);
  };
  ...
  // 3. Soumission du formulaire (Création OU Modif)
    ...
    if (conflicts.length >= 2) {
        alert("❌ Créneau saturé : Il ne peut pas y avoir plus de 2 rendez-vous simultanés.");
        return;
    }
  ...
  const eventStyleGetter = (event) => {
    ...
    return { style: {
      backgroundColor: colors.bg, color: colors.text, border: 'none', borderRadius: '4px',
      fontSize: '14px', fontWeight: '600', padding: '4px 8px', display: 'block'
    }};
  };
...
// --- 1. COMPOSANT POUR LA VUE SEMAINE / JOUR (Version Overlap-Friendly) ---
  ...
  const AgendaEventComponent = ({ event }) => (
    <div className="flex flex-row items-center justify-between w-full h-full">
      {/* Colonne 1 : Titre */}
      <div className="font-bold text-sm w-1/3 truncate">
        {event.title}
      </div>
  ...
  return (
    <div className="flex flex-1 overflow-hidden bg-gray-100 h-screen">
        ...
        <RightSidebar clients={clients} selectedClient={selectedClient} onChangeClient={setSelectedClient} />
